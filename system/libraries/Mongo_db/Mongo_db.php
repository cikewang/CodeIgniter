...
class Mongo_db extends CI_Driver_Library {
    ...
    public function __construct( $config = array() ) {
        ...
        require_once(BASEPATH.'libraries/Mongo_db/drivers/Mongo_db_driver.php');
        if(empty($this->drivers))
        {
            $this->driver = new Mongo_db_driver($this->_config);
        }

        require_once(BASEPATH.'libraries/Mongo_db/drivers/Mongo_db_result.php');
        if(empty($this->result))
        {
            $this->result = new Mongo_db_result();
        }
     ...
     * 
     * @param   string         The 文档字段
     * @param   mixed          The comparission使用价值
     * @return  void
     * @access  public
     */
     ...
     * @param  [type] $collection 查询集合
     * @param  string $where      查询条件
     * @return [type]             返回的数据
     */
    public function get_one($fields = '', $collection, $where = '')
    {
        if(is_string($fields) && !empty($fields))
        {
            $this->driver->select($fields);
        }
        ...
        return $this->_get_one( $collection);
    }
     ...
     * @access  public
     */
    public function get_where( $collection, $where = array(), $limit = null, $offset = null ) {
    ...
    private function _get( $collection, $limit = false, $offset = false ) {
        ...
        return $this->result->result_array( $result, $this->driver->db() );
    }

    private function _get_one($collection)
    {
       $result = $this->driver->get_one( $collection );
       return $this->result->result_array_one( $result);
    }
}
// END Mongo_db Class
